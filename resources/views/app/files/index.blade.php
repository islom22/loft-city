          ...
          <table class="table table-centered table-nowrap mb-0 rounded">
              ...
              <tbody>
                @foreach($files as $file)
                  <!-- Item -->
                  <tr>
                      <td><a href="#" class="text-primary fw-bold">{{ ($files ->currentpage()-1) * $files ->perpage() + $loop->index + 1 }}</a></td>
                      <td class="fw-bold">{{ $file->file }}</td>
                      <td>
                        <div class="actions d-flex">
                            <form class="" onclick="return myFunction();" action="{{ route('files.destroy', ['file' => $file]) }}" method="post">
                                @csrf
                                @method('delete')
